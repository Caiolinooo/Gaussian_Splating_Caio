...
> Última revisão: 2026-09-09 (14ª revisão — **qualidade + 4D/relight no L4, não contrato**. Viewer HTTPS `:2223`: Spark/WebGPU, nav única (Setup/Jobs/Upload/Viewer), órbita + LOD, relight `sh-env` com sliders vivos, scrubber temporal 154/220 no job `3ce6fee7`. Pipeline: 30k steps / `data_factor=2` / SH3 / exhaustive até 220 / paredes RANSAC + `ColmapDepthProvider` real / RIGS residual + `export/4dgs.npz`. Job `cf9a69b1` foi **apagado na UI** (DELETE 204) com o workdir. Job vivo `3ce6fee7`: 220 frames, 1º treino PSNR 39.17 / 124k gaussians / 66 cams (matcher sequential por bug em `_run_graph`); **rebuild SfM exhaustive a correr**. `ksplat` ainda 404 (sem splat-transform) — ply fallback.
> Revisão anterior: 2026-09-09 (13ª revisão — **hotfix auth remota**: dist em `:2222`/`:2223` tinha `VITE_DEV_AUTH_BYPASS=1` bakeado (`zb="1"` → Bearer `dev-bypass`, user `dev@localhost`). API em eval com `DEV_AUTH_BYPASS=0` respondia `Token inválido.` / `TOKEN_INVALID` e `UNAUTHENTICATED` sem token; lista de jobs do `caio` (ex. `cf9a69b1`) vinha vazia. Fix: bypass só em hostname loopback; `.env.production` força `VITE_DEV_AUTH_BYPASS=0`; 401 limpa `gs-local-auth` e manda para `/login` (sem banner morto em `/jobs`). Rebuild do dist no servidor. Sem mudança de pipeline/treino/renderer).
> Revisão anterior: 2026-09-09 (12ª revisão — **API no L4 verificada no browser**: `http://vm.groupabz.com:2222/setup` devolve saúde OK (NVIDIA L4 + CUDA 13.2, PyTorch 2.11+cu128, gsplat 1.5.3, COLMAP 4.3 CUDA). HTTPS para WebGPU: `https://vm.groupabz.com:2223`. O erro “uvicorn na porta 8000” vinha da UI Vite em `:5173` falando com localhost. Vite agora faz proxy same-origin para `:2222`; CORS do eval inclui `:5173`. Meshproxy deixa de travar o job: checa Open3D **antes** do KNN e pula nuvens >40k gaussianas. Workflow CUDA (SfM GPU + treino gsplat) já concluiu job de vídeo no server. Viewer: Spark/WebGPU (HTTPS) com fallback WebGL2).
> Revisão anterior: 2026-09-08 (11ª revisão — **contrato das 4 inspirações** no SceneIO já no ar: SuperSplat 3, GaussianCrowds, Video2 4DGS, paredes-sem-LiDAR + depth forward-pass. Tabela única em `docs/unified-scene-io.md`. `temporal` já cobre timesteps de vídeo/GIF (`sourceKind: video\|gif`); `.npz`/Comfy e `timestampsS` ficam reservados — sem `source_kind` `comfy`. Slots `dataset/depths/` + `export/proxy.glb` documentados; **TODO** `ColmapDepthProvider` continua stub. Briefings: `docs/inspiration-video24dgs.md`, `docs/inspiration-walls-depth.md`. Sem novo pipeline).
> Revisão anterior: 2026-09-08 (10ª revisão — **SceneIO unificado**: uma superfície `ingest_scene`/`export_scene` para PLY, GIF, vídeo e imagens. PLY pula SfM/treino/meshproxy/autocal; GIF → frames (ffmpeg); export grava `scene.json` + `scene.zip` além de `.ply`/`.ksplat`. Schema de cena ganha `temporal` + `relight` (contrato 4DGS/relight, sem fingir GaussianCrowds). Viewer: badge WebGPU, scrubber, download de cena. Ver `docs/unified-scene-io.md`. COLMAP/SfM da 9ª revisão permanece intacto).
...
- [x] Marco 0 (parcial): contratos `temporal`/`relight` no JSON de cena + scrubber no HUD + ingestão GIF como sequência de frames. **Não** há treino 4DGS/relightable (GaussianCrowds permanece pesquisa). Video2 4DGS entra como o mesmo `temporal` (vídeo/GIF); interchange `.npz` documentado, ComfyUI fora do Provisioner. Paredes/depth: slots `dataset/depths/` + `export/proxy.glb` no contrato; `ColmapDepthProvider` ainda stub (sem Spirula).
