...
`docs/unified-scene-io.md` e `docs/inspiration-video24dgs.md` **não estavam no tree** quando este briefing foi escrito. A linguagem abaixo alinha-se ao que já existe (`tasks.md`, `SceneDocument` / `CalibrationJson`, `pipeline/meshproxy`, `pipeline/jobs/autocal.py` + `ColmapDepthProvider`) e ao contrato planejado `ingestScene` / `exportScene`. Se esses docs forem criados depois, este texto deve ser citado — não contradito.

**Regra de produto (repetida):** depth e paredes são **artefatos da mesma cena** (mapa de profundidade, malha proxy). Não viram `source_kind` novo. Hoje `source_kind` é só `video` | `images` (ingestão). SuperSplat 3 / GaussianCrowds 4D / Video2 4DGS entram como **formatos de interchange** do mesmo documento, não como jobs irmãos.
...
e o documento de cena continua `backgroundSplat` + `nodes` + `calibration` + `overlays` (`packages/viewer/src/editing/sceneSchema.ts`, `apps/api/app/schemas/scenes.py`).
...
- **RealityScan / Metashape como dependência.** O OP já aceita COLMAP. Nosso SfM é COLMAP (`source_kind` video/images). RealityScan não entra no Provisioner.
...
- **Segundo renderer.** PlayCanvas / SuperSplat / SuperSplat Viewer ficam no **interchange** (`ingestScene` / `exportScene`), não substituem Spark + `@mkkellogg` atrás de `SplatRenderer` (`tasks.md` §7 decisão 1).
- **Segundo treino / segundo `.ply`.** Um `master.ply`, um `scene.ksplat`, uma proxy. Depth/normal são insumos ou saídas do **mesmo** grafo.
- **Novo `source_kind`.** Não criar `lidar`, `depth`, `spirula`, `supersplat`, `4dgs`. Interchange estrangeiro vira a **mesma** `SceneDocument`.
...
Vocabulário planejado (quando `docs/unified-scene-io.md` existir, usar os mesmos nomes):

| Símbolo | Papel |
|---------|--------|
| `SceneIO` | Orquestrador único: um documento, muitos formatos de ficheiro. |
| `ingestScene` | SuperSplat 3 (WebGPU / compute GSplat), GaussianCrowds (4D + relight), Video2 (4DGS + timesteps), **e** o job nativo vídeo/imagens → o mesmo `SceneDocument`. |
| `exportScene` | Grava o mesmo documento + artefatos de disco; não escolhe um “sabor” de cena. |

`source_kind` permanece o da **captura** (`video` | `images`). 4D/relight/WebGPU são **capítulos do documento** (trilha temporal, probes, backend de raster), não tipos de ingestão.

### Artefatos da mesma cena (não ramos)

Layout já usado por `JobPaths` + extensões **opcionais**:
...
{work_dir}/
  ...
  dataset/
    images/                    # (já)
    depths/                    # NOVO, opcional — prior monocular ou ED bakeado
    normals/                   # NOVO, opcional — prior monocular
  export/
    master.ply                 # splat canónico
    scene.ksplat               # entrega web
    proxy.glb                  # meshproxy (overlays)
    calibration.json           # autocal → CalibrationJson
    depth/                     # NOVO, opcional — mapas ou buffer bakeado p/ SceneIO
```

No JSON de cena (`schemaVersion` atual = 1), depth/proxy **não** viram `SceneNodeKind` novo. A proxy já é o alvo implícito dos overlays (e, quando visível, um nó `glb` de trabalho). Depth é metadado/artefato:
...
SceneDocument
  ...
  artifacts?          → { proxyGlb, depthMaps, normalMaps }   # contrato futuro; mesmo id de cena
```

`ingestScene` (formatos planejados):

- **SuperSplat 3 WebGPU** — lê splat + `settings.json` do [supersplat-viewer](https://github.com/playcanvas/supersplat-viewer) (já tem `sceneDepthMap` no renderer compute). Depth attachment e collision `.glb` do viewer oficial **mapeiam** para `artifacts.depthMaps` / `artifacts.proxyGlb`. Não instanciar o engine PlayCanvas no runtime nosso.
- **GaussianCrowds 4D / relight** — timesteps + probes de luz no **mesmo** documento (Fase 6 / player temporal já previsto em `SplatRenderer.play/pause`). Depth/paredes são por-frame da **mesma** trilha, não um job “crowd”.
- **Video2 4DGS** — timestamps de frame (já temos ingest de vídeo) → campo temporal do documento. Um `depths/` por timestep, se existir; `source_kind` continua `video`.

`exportScene` escreve o que o job já produz (`master.ply`, `scene.ksplat`, `proxy.glb`, `calibration.json`) e, quando presentes, os mapas. Um import SuperSplat/Crowds/Video2 **não** gera um segundo grafo de treino só para paredes.
...
### Contrato futuro (quando `SceneIO` / `ingestScene` / `exportScene` existirem)

1. **`ingestScene` / `exportScene`** normalizam SuperSplat 3, GaussianCrowds 4D/relight e Video2 4DGS para o mesmo `SceneDocument` + os artefatos da tabela acima. Depth e proxy viajam com a cena; o viewer não pergunta “de que pipeline veio”.
2. **Prior no `training` gsplat** (Apache-2.0, já escolhido): loss de depth/normal estilo DN-Splatter sobre mapas Metric3D (ou equivalente permissivo) gerados **depois do SfM**, no worker, como passo opcional **dentro** de `training` ou como pré-passo barato do dataset — ainda no mesmo job. Sem binário GPLv3.
...
4. **`SplatRenderer`:** attachment de depth linear no forward pass (média de recíprocos ou expected), para overlays/`depthTest` e efeitos. Continua Spark | MkKellogg. SuperSplat 3 é formato de **ficheiro**, não backend.
5. **4D:** cada timestep reusa o mesmo par (splat, depth, proxy). Relight (Crowds) não duplica geometria de parede.
...
- Renderer: `SplatRenderer` = Spark | MkKellogg; SuperSplat **não** é o runtime (`tasks.md` §6–§7).
- `source_kind`: só ingestão vídeo/imagens (`pipeline/sfm`, upload API).
